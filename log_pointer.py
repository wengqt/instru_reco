...
def findContours(findContoursImg,dstCanny,heartsarr=None,dst=None,offset1=0,offset2=0,big_index=0):
    ...
    cv2.imwrite(dir_path+'/cut_findContours.jpg', new_img_canny)
    _heart= None
...
def findHearts(src,org):
    ...
    cv2.imwrite(dir_path+'/2_cut_find_heart.jpg',src)
    ...
    cv2.imwrite(dir_path+'/2_cut_Circles.jpg',cuto_cp)
...
def findMainZone(path):
    ...
    gray_o = canny[d1:the_circle[1] + the_circle[2],
               d2:the_circle[0] + the_circle[2]]
    cv2.imwrite(dir_path+'/1_cut.jpg', cutImg_o)
    ...
    cv2.imwrite(dir_path+'/1_cut_adapt.jpg', cutImg3)
...
def convertPolar(zone,_heart,type,zone2=None):
    ...
    # cv2.imwrite(dir_path+'/6_cut_numZoneCanny.jpg', zone)
    ...
    cv2.imwrite(dir_path+'/6_nums_kedu'+str(type)+'.jpg', area)  # 直着的 带刻度带数字图
    ...
    if type==1:
        ...
        cv2.imwrite(dir_path+'/6_nums_kedu'+str(type)+'.jpg', non_area)  # 直着的 带刻度带数字图
        print('img', dir_path+'/6_nums_kedu'+str(type)+'.jpg')
        try:
            if len(border) < 3:
                raise Exception('border less than 3 ')
    ...
    cv2.imwrite(dir_path+'/6_nums_'+str(type)+'.jpg', _num)  # 直着的 带刻度带数字图
    print('img', dir_path+'/6_nums_'+str(type)+'.jpg')
    cv2.imwrite(dir_path+'/6_kedu_'+str(type)+'.jpg', _kedu)  # 直着的 带刻度带数字图
...
def processNum(cnn,numarea,k_rate,index):
    ...
    cv2.imwrite(dir_path+'/9_numsdilate'+str(index)+'.jpg', numsdilate)
...
def getScaleArea(heart_arr,_img,non_img_1,_non,theta_):
    ...
    cv2.imwrite(dir_path+'/5_area1.jpg', _img1)
    print('img', dir_path+'/5_area1.jpg')

    _img2 = cv2.bitwise_and(blank_img2, non_img_1)
    cv2.imwrite(dir_path+'/5_area2.jpg', _img2)
    ...
    cv2.imwrite(dir_path+'/5_cut_dilate1.jpg', eroded1)
    ...
    cv2.imwrite(dir_path+'/5_cut_dilate2.jpg', eroded2)
    ...
    cv2.imwrite(dir_path+'/5_cut_res2.jpg', _cutnumZone2)
    print('img', dir_path+'/5_cut_res2.jpg')
    cv2.imwrite(dir_path+'/5_cut_p_img1.jpg', cut_non1)
    print('img', dir_path+'/5_cut_p_img1.jpg')
    cv2.imwrite(dir_path+'/5_cut_p_img2.jpg', cut_non2)
...
if __name__ == '__main__':
    ...
    try:
        ...
        dir_path_arr = img_path.split(os.path.sep)[:-1]
        filename = img_path.split(os.path.sep)[-1].split('.')[0]
        dir_path = os.path.sep.join(dir_path_arr)+'/'+filename
        isExists = os.path.exists(dir_path)
        if not isExists:
            os.makedirs(dir_path)
            os.makedirs(dir_path + '/num1')
